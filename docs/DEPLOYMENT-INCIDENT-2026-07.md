...
| DATABASE_CLIENT 配置错误 | 旧版强制独立 PostgreSQL，增加部署复杂度 | 新版强制 DATABASE_CLIENT=mysql 并与 CRM 共库；旧 PostgreSQL 只在迁移窗口读取 |
...
API、静态资源和 Nginx 运行时验收。失败时恢复旧版本、配置和服务；业务数据库
不会被脚本自动恢复，升级前仍必须制作可恢复备份。
