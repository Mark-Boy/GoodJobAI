...
for required_build in \
  ...
  "$PROJECT_ROOT/whatsapp-plugin/dist-server/server/index.js" \
  "$PROJECT_ROOT/whatsapp-plugin/dist-server/server/scripts/migrate.js" \
  "$PROJECT_ROOT/whatsapp-plugin/dist-server/server/scripts/migrate-postgres-to-mysql.js"; do
...
cp "$PROJECT_ROOT/docs/MYSQL-UNIFIED-DATABASE-MIGRATION.md" "$STAGING_DIR/docs/MYSQL-UNIFIED-DATABASE-MIGRATION.md"
...
grep -Fq 'dist-server/server/scripts/migrate.js' "$STAGING_DIR/install-baota.sh" \
  || { printf '安装包缺少 Communication 生产迁移入口\n' >&2; exit 1; }
grep -Fq 'MYSQL_DATA_IMPORT_TOKEN' "$STAGING_DIR/install-baota.sh" \
  || { printf '安装包缺少 MySQL 文件迁移授权配置\n' >&2; exit 1; }
grep -Fq 'APP_DATABASE_PROFILE=production' "$STAGING_DIR/install-baota.sh" \
  || { printf '安装包缺少生产数据库档位保护\n' >&2; exit 1; }
for private_path in ".env.*.local" "uploads/" "dist-packages/"; do
  grep -Fq -- "--exclude='$private_path'" "$STAGING_DIR/install-baota.sh" \
    || { printf '安装包升级复制缺少私有路径排除：%s\n' "$private_path" >&2; exit 1; }
done
if ! grep -Eq 'MySQL 数据迁移|选择 MySQL 文件|数据库维护' "$STAGING_DIR/frontend/dist/index.html"; then
  printf '安装包缺少 MySQL 数据迁移界面\n' >&2
  exit 1
fi
