...
check_runtime() {
  ...
  if ! command -v soffice >/dev/null 2>&1 && ! command -v libreoffice >/dev/null 2>&1; then
    warn "未检测到 LibreOffice Calc；正式安装阶段将自动安装，用于 Excel 原版式预览"
  fi
  ...
  printf '运行环境：Node %s / npm %s / Communication 与 CRM 共用 MySQL\n' \
    "$("$node_bin" -v)" "$("$npm_bin" -v)"
}
...
verify_environment() {
  ...
  grep -Fxq 'DATABASE_CLIENT=mysql' "$plugin_env" \
    || { die "Communication 生产环境必须配置 DATABASE_CLIENT=mysql"; return 1; }
  [[ "$(sed -n 's/^DATABASE_URL=//p' "$plugin_env" | tail -n 1)" \
    == "$(sed -n 's/^DATABASE_URL=//p' "$crm_env" | tail -n 1)" ]] \
    || { die "Communication 与 CRM 必须复用同一个 MySQL DATABASE_URL"; return 1; }
}
...
5. Communication 未与 CRM 复用同一个 MySQL 数据库。
...
identify_known_failure() {
  ...
  elif [[ -f "$source_file" ]] && grep -Eqi 'DATABASE_CLIENT must be mysql|Communication.*MySQL' "$source_file"; then
    KNOWN_FAILURE_HINT="Communication 未正确配置为 CRM 共用 MySQL"
