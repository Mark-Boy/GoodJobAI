...
WHATSAPP_LEGACY_DATABASE_URL="${WHATSAPP_LEGACY_DATABASE_URL:-}"
DOMAIN="${DOMAIN:-}"
...
INBOUND_MAIL_ENABLED="${INBOUND_MAIL_ENABLED:-true}"
...
verify_package() {
  ...
  for required_file in \
    ...
    "$SOURCE_DIR/whatsapp-plugin/dist-server/server/scripts/migrate.js" \
    "$SOURCE_DIR/whatsapp-plugin/dist-server/server/scripts/migrate-postgres-to-mysql.js" \
    "$SOURCE_DIR/LICENSE" \
    "$SOURCE_DIR/NOTICE" \
...
if ! command -v soffice >/dev/null 2>&1 && ! command -v libreoffice >/dev/null 2>&1; then
  if command -v apt-get >/dev/null 2>&1; then
    log "安装 Excel 原版式预览依赖"
    export DEBIAN_FRONTEND=noninteractive
    apt-get update
    apt-get install -y libreoffice-calc libreoffice-common fonts-noto-cjk
  else
    die "Excel 预览需要 LibreOffice Calc；请先安装 soffice/libreoffice 后重试"
  fi
fi
LIBREOFFICE_BIN="$(command -v soffice 2>/dev/null || command -v libreoffice 2>/dev/null || true)"
[[ -n "$LIBREOFFICE_BIN" ]] || die "LibreOffice 安装完成后仍未找到可执行文件"
...
legacy_whatsapp_database_url="$WHATSAPP_LEGACY_DATABASE_URL"
if [[ -z "$legacy_whatsapp_database_url" \
  && -f "$WHATSAPP_PLUGIN_ENV_FILE" \
  && "$(sed -n 's/^DATABASE_CLIENT=//p' "$WHATSAPP_PLUGIN_ENV_FILE" | tail -n 1)" == "postgres" ]]; then
  legacy_whatsapp_database_url="$(sed -n 's/^DATABASE_URL=//p' "$WHATSAPP_PLUGIN_ENV_FILE" | tail -n 1)"
fi
if [[ -n "$legacy_whatsapp_database_url" \
  && "$legacy_whatsapp_database_url" != postgres://* \
  && "$legacy_whatsapp_database_url" != postgresql://* ]]; then
  die "旧 Communication 数据库连接必须是 PostgreSQL URL"
fi
...
runuser -u "$APP_USER" -- env \
  HOME="/var/lib/$APP_USER" \
  PATH="$node_path:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin" \
  APP_DATABASE_PROFILE=production \
...
if [[ "$https_active" == true ]]; then
  ...
  email_tracking_base_url="https://$DOMAIN"
else
...
EMAIL_TRACKING_SECRET=$EMAIL_TRACKING_SECRET
...
INBOUND_MAIL_ENABLED=$INBOUND_MAIL_ENABLED
INBOUND_MAIL_POLL_MS=120000
INBOUND_MAIL_LOOKBACK_DAYS=7
INBOUND_MAIL_MAX_MESSAGES_PER_POLL=100
INBOUND_MAIL_MAX_SOURCE_BYTES=2097152
EMAIL_TRACKING_BASE_URL=$email_tracking_base_url
EMAIL_MESSAGE_ID_DOMAIN=$DOMAIN
OUTBOUND_EMAIL_DAILY_LIMIT=50
OUTBOUND_EMAIL_DOMAIN_WINDOW_MINUTES=10
OUTBOUND_EMAIL_DOMAIN_WINDOW_LIMIT=5
OUTBOUND_EMAIL_BOUNCE_WINDOW_DAYS=7
OUTBOUND_EMAIL_BOUNCE_MIN_SAMPLE=20
OUTBOUND_EMAIL_MAX_BOUNCE_PERCENT=10
OUTBOUND_EMAIL_APPEND_OPTOUT=true
OUTBOUND_EMAIL_REQUIRE_DNS_AUTH=false
EMAIL_DKIM_SELECTORS=
EOF
...
set -a
# 迁移脚本读取插件环境文件，确保 Communication 与 CRM 使用同一个 MySQL 数据库。
source "$WHATSAPP_PLUGIN_ENV_FILE"
set +a
runuser -u "$APP_USER" -- env \
  HOME="/var/lib/$APP_USER" \
  PATH="$node_path:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin" \
  "$NODE_BIN" "$RELEASE_DIR/whatsapp-plugin/dist-server/server/scripts/migrate.js"

if [[ -n "$legacy_whatsapp_database_url" ]]; then
  log "迁移旧 Communication PostgreSQL 数据到统一 MySQL，并逐表校验"
  runuser -u "$APP_USER" -- env \
    HOME="/var/lib/$APP_USER" \
    PATH="$node_path:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin" \
    SOURCE_DATABASE_URL="$legacy_whatsapp_database_url" \
    DATABASE_URL="$database_url" \
    "$NODE_BIN" "$RELEASE_DIR/whatsapp-plugin/dist-server/server/scripts/migrate-postgres-to-mysql.js" \
      --apply --resume-completed
  log "旧 PostgreSQL 数据已完整迁入 MySQL；旧库未删除，仅保留作人工回滚来源"
fi
