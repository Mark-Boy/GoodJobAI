...
import { canSeeOwner, hasIamPermission, type IamActor } from "./auth.js";
import type { CrmStore } from "./store.js";
import type { AiModelConfig, SalesDistillation, SalesDistillationMetrics, SessionUser, User } from "./types.js";

type DistillationActor = IamActor;

function canManageTeam(actor: DistillationActor) {
  return hasIamPermission(actor, "training.manage");
}

function canAccessSource(actor: DistillationActor, source: User) {
  return source.id === actor.id || (canManageTeam(actor) && canSeeOwner(actor, source.id, source.teamId));
}
...
export function listSalesDistillations(store: CrmStore, actor: DistillationActor) {
  return store.salesDistillations.filter((item) => item.createdBy === actor.id
    || (item.status === "published" && item.teamId === actor.teamId)
    || (canManageTeam(actor) && canSeeOwner(actor, item.sourceUserId, item.teamId)))
...
export async function publishSalesDistillation(store: CrmStore, actor: DistillationActor, id: string) {
  ...
  if (!item || !(item.createdBy === actor.id || (canManageTeam(actor) && canSeeOwner(actor, item.sourceUserId, item.teamId)))) throw new Error("蒸馏记录不存在或无权访问");
...
export async function activateSalesPlaybook(store: CrmStore, actor: DistillationActor, distillationId: string) {
  ...
  if (!distillation || distillation.status !== "published" || distillation.teamId !== actor.teamId) {
...
export async function pauseSalesPlaybook(store: CrmStore, actor: DistillationActor, activationId: string) {
  const activation = store.salesPlaybookActivations.find((item) => item.id === activationId && item.ownerId === actor.id && item.teamId === actor.teamId);
...
export function activeSalesPlaybookContext(store: CrmStore, actor: DistillationActor) {
  const activation = store.salesPlaybookActivations
    .filter((item) => item.ownerId === actor.id && item.status === "active" && item.teamId === actor.teamId)
